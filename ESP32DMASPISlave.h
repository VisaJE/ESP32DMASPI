...
class Slave
{
    ...
    spi_bus_config_t bus_cfg{
        ...
        .sclk_io_num = 14, // HSPI
        .max_transfer_sz =
            4092, // default: 4092 if DMA enabled, SOC_SPI_MAXIMUM_BUFFER_SIZE if DMA disabled
  ...
  public:
    ...
    bool end() { return (spi_slave_free(host) == ESP_OK); }
    ...
    bool wait(uint8_t *rx_buf, const size_t size) { return wait(rx_buf, NULL, size); }

    bool wait(uint8_t *rx_buf, const uint8_t *tx_buf, const size_t size)
    {
        if (!transactions.empty())
        {
            printf("[WARN] cannot execute transfer if queued transaction exists. queued "
                   "transactions = %d\n",
                   transactions.size());
            return 0;
        }
    ...
    bool queue(uint8_t *rx_buf, const size_t size) { return queue(rx_buf, NULL, size); }
    bool queue(uint8_t *rx_buf, const uint8_t *tx_buf, const size_t size)
    {
        if (transactions.size() >= if_cfg.queue_size)
        {
            printf("[WARNING] slave queue is full with transactions. discard new transaction "
                   "request\n");
            return false;
        }
    ...
    size_t available() const { return results.size(); }

    // size of queued (not completed) transactions
    size_t remained() const { return transactions.size(); }

    // size of the received bytes of the oldest queued transaction result
    uint32_t size() const { return results.front() / 8; }

    // pop the oldest transaction result
    void pop() { results.pop_front(); }

    // ===== Main Configurations =====
    // set these optional parameters before begin() if you want

    void setDataMode(const uint8_t m) { setSpiMode(m); }

    void setMaxTransferSize(const int n) { bus_cfg.max_transfer_sz = n; }

    // ===== Optional Configurations =====

    void setSlaveFlags(const uint32_t flags) { if_cfg.flags = flags; }

    void setQueueSize(const int n) { if_cfg.queue_size = n; }

    void setSpiMode(const uint8_t m) { if_cfg.mode = m; }

    void setDMAChannel(const uint8_t c)
    { // default: auto
        if ((1 <= c) && (c <= 3))
        {
            dma_chan = c;
        ...
        {
            printf("[WARN] invalid DMA channel %d, force to set auto select. make sure to select 1 "
                   "or 2\n",
                   c);
  ...
  private:
    ...
    void pushResult(const uint32_t s) { results.push_back(s); }
};
